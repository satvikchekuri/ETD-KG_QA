...
def ingest_etds_batch(tx, rows):
    tx.run("""
        UNWIND $rows AS row
        MERGE (e:ETD {etdId: row.etdId})
        SET e.URI = row.URI

        MERGE (t:Title {text: row.title})
        SET t.abstract_text = row.abstract

        MERGE (a:Author {name: row.author})
        MERGE (d:Department {name: row.department})
        MERGE (adv:Advisor {name: row.advisor})
        MERGE (u:University {name: row.university})
        MERGE (y:Year {value: row.year})
        MERGE (dg:Degree {name: row.degree})
        MERGE (tp:Topic {label: row.topic})
...
def ingest_figures_batch(tx, rows):
    tx.run("""
        UNWIND $rows AS row
        MERGE (f:Figure {
            etdId: row.etdId,
            ...
            caption_text: row.figure_caption
        })
        WITH f, row
        MATCH (e:ETD {etdId: row.etdId})
        MERGE (e)-[:hasFigure]->(f)
        MERGE (f)-[:figureOf]->(e)
    """, rows=rows)


def ingest_tables_batch(tx, rows):
    tx.run("""
        UNWIND $rows AS row
        MERGE (t:Table {
            etdId: row.etdId,
            ...
            caption_page_no: row.caption_page_no,
            caption_text: row.table_caption
        })
        WITH t, row
        MATCH (e:ETD {etdId: row.etdId})
        MERGE (e)-[:hasTable]->(t)
...
def ingest_sentences_batch(tx, rows):
    tx.run("""
        UNWIND $rows AS row
        MERGE (s:AbstractSentence {
            etdId: row.etdId,
            sentence: row.sentence,
            classified_label: row.label
        })
        WITH s, row
        MATCH (e:ETD {etdId: row.etdId})
...
def create_indexes(session):
    index_queries = [
        ...
        "CREATE INDEX IF NOT EXISTS FOR (t:Title) ON (t.text)",
        "CREATE INDEX IF NOT EXISTS FOR (s:AbstractSentence) ON (s.etdId, s.sentence)",
...
def create_indexes_fulltext(session):
...
def main():
    ...
    with driver.session() as session:
        ...
        for batch_df in tqdm(batch_rows(df_etd, 100), desc="ETDs"):
            rows = [{
                "etdId": str(row["ID"]),
                "URI": row["URI"],
            ...
            session.execute_write(ingest_etds_batch, rows)

        for batch_df in tqdm(batch_rows(df_figures, 100), desc="Figures"):
            rows = [{
                "etdId": str(row["ID"]),
                "figure_id": row["figure_id"],
                "figure_path": row["figure_path"],
            ...
            session.execute_write(ingest_figures_batch, rows)

        for batch_df in tqdm(batch_rows(df_tables, 100), desc="Tables"):
            rows = [{
                "etdId": str(row["ID"]),
                "table_id": row["table_id"],
                "table_path": row["table_path"],
            ...
            session.execute_write(ingest_tables_batch, rows)

        for batch_df in tqdm(batch_rows(df_abstract, 100), desc="Abstract Sentences"):
            rows = [{
                "etdId": str(row["ID"]),
                "sentence": row["sentence"],
                "label": row["classified_label"]
            } for _, row in batch_df.iterrows()]
            session.execute_write(ingest_sentences_batch, rows)

        create_indexes_fulltext(session)
